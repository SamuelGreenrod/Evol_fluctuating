...
ggsave("Fig_1a.png", width = 8, height = 3, dpi = 300)



# Number of generations and effective population size

pfu <- read_csv("Data/PFU_values_each_passage_all.csv")
...
df_test <- pfu %>%
  ...
  mutate(
    prev_pfu = lag(pfu_ml),
    progeny = pfu_ml - prev_pfu) %>%
  ungroup()%>%
  filter(passage %% 1 == 0)


ggplot(df_test %>% filter(phage_measured == "phage14_1"), aes(x = passage, y = progeny, group = replicate))+
  geom_line()+
  scale_y_continuous(trans='log10', limits = c(0.01,10^12)) +
  
  facet_grid(phage_treatment ~ temperature_treatment)


total_generations <- df_test %>%
  filter(passage <= 15) %>%
  group_by(
    phage_measured,
    phage_treatment,
  ...
  summarise(
    total_progeny = sum(progeny, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(evol_comp = paste(temperature_treatment, phage_treatment, sep = "_"))


total_generations$Legend_group1 <- dplyr::case_when(
  total_generations$evol_comp == "37C_static_Single_phage" ~ "37°C\nStatic\nMono.",
  total_generations$evol_comp == "42C_static_Single_phage" ~ "42°C\nStatic\nMono.",
  total_generations$evol_comp == "37C_static_Competition" ~ "37°C\nStatic\nCo.",
  total_generations$evol_comp == "42C_static_Competition" ~ "42°C\nStatic\nCo.",
  total_generations$evol_comp == "Fluctuating_Single_phage" ~ "Fluc.",
  total_generations$evol_comp == "Fluctuating_Competition" ~ "Fluc.\nCo.",
)
total_generations$Legend_group2 <- dplyr::case_when(
  total_generations$evol_comp == "37C_static_Single_phage" ~ "37°C Static (Monoculture)",
  total_generations$evol_comp == "42C_static_Single_phage" ~ "42°C Static (Monoculture)",
  total_generations$evol_comp == "37C_static_Competition" ~ "37°C Static (Co-culture)",
  total_generations$evol_comp == "42C_static_Competition" ~ "42°C Static (Co-culture)",
  total_generations$evol_comp == "Fluctuating_Single_phage" ~ "Fluctuating (Monoculture)",
  total_generations$evol_comp == "Fluctuating_Competition" ~ "Fluctuating (Co-culture)",
)
...
total_generations$phage_measured <- factor(total_generations$phage_measured, levels = c("phage14_1", "LUZ19"), labels = c("ϕ14-1", "ϕLUZ19"))
total_generations$Legend_group1 <- factor(total_generations$Legend_group1, levels = c("37°C\nStatic\nMono.", "37°C\nStatic\nCo.", "Fluc.", "Fluc.\nCo.", "42°C\nStatic\nMono.", "42°C\nStatic\nCo."))


ggplot(total_generations, aes(x = Legend_group1, y = total_progeny, color = Legend_group2)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  geom_point(aes(group = phage_treatment), 
             position = position_dodge(width = 0.8), size = 2) +
  theme_bw() +
  #ylim(50, 60) +
  facet_grid(~ phage_measured) +
  labs(
    x = "Evolution treatment",
    y = "Total number of phage progeny produced",
    color = ""
  ) +
  ...
  scale_y_continuous(trans='log10', limits = c(10^10,10^12)) +
  annotation_logticks(sides="l")+
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
luz19_stats = total_generations %>%
  filter(phage_measured == "ϕLUZ19")

p14_stats = total_generations %>%
  filter(phage_measured == "ϕ14-1")

luz19_stats %>%
  group_by(Legend_group2) %>%
  summarise(mean_prog = mean(total_progeny, na.rm = TRUE))

p14_stats %>%
  group_by(Legend_group2) %>%
  summarise(mean_prog = mean(total_progeny, na.rm = TRUE))

mod = lm(log(total_progeny) ~ phage_treatment * temperature_treatment, data = luz19_stats)
joint_tests(mod)
summary(mod)
plot(mod)
emmeans(mod, pairwise ~ phage_treatment * temperature_treatment)


mod = lm(log(total_progeny) ~ phage_treatment * temperature_treatment, data = p14_stats)
joint_tests(mod)
summary(mod)
anova(mod)
plot(mod)
...
phage_fitness_pfu_update$temp <-  factor(phage_fitness_pfu_update$temperature, levels = c("37","42"), labels = c("37\u00B0C", "42\u00B0C"))
...
phage_fitness_pfu_update_boxplots_single_normalised$line = factor(phage_fitness_pfu_update_boxplots_single_normalised$line, levels = c("37°C static", "Fluctuating", "42°C static"), labels = c("37°C\n(static)", "Fluctuating", "42°C\n(static)"))

label_map <- c(
  "ϕ14-1 (37°C)" = "ϕ14-1 (37°C)",
  "ϕ14-1 (42°C)" = "ϕ14-1 (42°C)",
  "ϕLUZ19 (37°C)" = "ϕLUZ19 (37°C)",
  "ϕLUZ19 (42°C)" = "ϕLUZ19 (42°C)"
)
...
ggplot(phage_fitness_pfu_update_boxplots_single_normalised, aes(line, PFU_relative, colour = as.factor(line))) +
  ...
  xlab("Evolution treatment (monoculture only)")+
...
ggsave("New_figures/Fig_1b.png", width = 8, height = 4, dpi = 300)
...
mod = lme4::lmer(log10(pfu_ml) ~ line * temp + (1|batch), data = luz19_stats)
joint_tests(mod)
summary(mod)
plot(mod)
...
mod = lme4::lmer(log10(pfu_ml+1) ~ line * temp + (1|batch), data = p14_stats) # 1 added to allow log transformation
joint_tests(mod)
...
phage_fitness_pfu_update_boxplots = phage_fitness_pfu_update_comp %>%
  dplyr::filter((phage == "ϕ14-1" & time == "4") |
                  (phage == "ϕLUZ19" & time == "2"))   # Remove T2 values from 14-1 dataset as only carried out for a single rep
...
phage_fitness_pfu_update_boxplots_normalised$phage_temp = factor(phage_fitness_pfu_update_boxplots_normalised$phage_temp, levels = c("ϕ14-1_37°C", "ϕ14-1_42°C", "ϕLUZ19_37°C", "ϕLUZ19_42°C"), labels = c("ϕ14-1 (37°C)", "ϕ14-1 (42°C)", "ϕLUZ19 (37°C)", "ϕLUZ19 (42°C)"))
...
ggplot(phage_fitness_pfu_update_boxplots_normalised, aes(Legend_group1, PFU_relative, color = as.factor(Legend_group2))) +
  ...
  ylab("Phage growth rel. to ancestor") +
  xlab("Evolution treatment (monoculture and co-culture)")+
  labs(colour="Evolution treatment")+
...
ggsave("New_figures/Fig_1c.png", width = 8, height = 4, dpi = 300)


phage_fitness_pfu_update_boxplots = phage_fitness_pfu_update %>%
  filter((phage == "ϕ14-1" & time == "4") |
           (phage == "ϕLUZ19" & time == "2"))
...
mod = lme4::lmer(log10(pfu_ml) ~ line * temperature + (1|batch), data = luz19_stats)
joint_tests(mod)
summary(mod)
plot(mod)
emmeans(mod, pairwise ~ line | temperature )


mod = lme4::lmer(log(pfu_ml+1) ~ line * temperature + (1|batch), data = p14_stats) # 1 added to allow log transformation
joint_tests(mod)
...
tree_nc_14 <- ape::nj(phage14_distance_matrix_nc)
tree_rooted_nc_14 <- ape::root(tree_nc_14, outgroup = "14-1-anc", resolve.root = TRUE)
...
phage14_eucl_tree = ggtree::ggtree(tree_rooted_nc_14, layout = "rectangular") %>%
...
tree_nc_L <- ape::nj(luz19_distance_matrix_nc)
tree_rooted_nc_L <- ape::root(tree_nc_L, outgroup = "LUZ19-anc", resolve.root = TRUE)
...
ggsave("Fig_2a.png", width = 8, height = 4, dpi = 300)
...
ggsave("Fig_2b.png", width = 8, height = 3, dpi = 300)
...
# Venn plot
breseq_14_filtered2 <- breseq_14_filtered %>%
  mutate(BC_key = paste(type, position, sep = "_"))

set_A <- breseq_14_filtered2 %>%
  filter(evol_group == "14-1-37") %>%
  pull(BC_key) %>%
  unique()

set_B <- breseq_14_filtered2 %>%
  filter(evol_group == "14-1-42") %>%
  pull(BC_key) %>%
  unique()

set_C <- breseq_14_filtered2 %>%
...
phage14_shared = ggVennDiagram(venn_list, label_alpha = 0) +
...
luz19_tbl <- xtabs(count ~ Evolved_line + Mutation, data = L_mutation_counts_long)
chisq.test(luz19_tbl)
fisher.test(luz19_tbl)

all_snps <- p14_mutation_counts_long %>% 
  mutate(luz19_count = L_mutation_counts_long$count,
         all_snps_count = count + luz19_count)
...
# Venn diagram plots
breseq_L_filtered2 <- breseq_L_filtered %>%
  mutate(BC_key = paste(type, position, sep = "_"))

set_A <- breseq_L_filtered2 %>%
  filter(evol_group == "LUZ19-37") %>%
  pull(BC_key) %>%
  unique()

set_B <- breseq_L_filtered2 %>%
  filter(evol_group == "LUZ19-42") %>%
  pull(BC_key) %>%
  unique()

set_C <- breseq_L_filtered2 %>%
...
# Plot Venn diagram
luz19_shared = ggVennDiagram(venn_list, label_alpha = 0) +
  scale_fill_gradient(low = "white", high = "darkgreen",
                      limits = c(0, 15)
  )+
  theme_void()

phage14_shared + luz19_shared
...
ggsave("Fig_4A.png", width = 4, height = 3, dpi = 300)
...
# Number of mutations in each population
...
breseq14_summary = breseq_14_filtered %>%
  count(phage) %>%
  mutate(
    evol_group = case_when(
      grepl("37", phage) ~ "37°C static",
      grepl("42", phage) ~ "42°C static",
      grepl("F",phage) ~ "Fluctuating",
      TRUE               ~ NA_character_
    )
  )

cbPalette3 <- c("37°C static" = "#377EB8", 
                "Fluctuating" = "#6A3D9A", 
                "42°C static" = "#E41A1C")

breseq14_summary$phage <- factor(breseq14_summary$phage, levels = c("14-1-42-1", "14-1-42-2", "14-1-42-3", "14-1-42-4", "14-1-42-5", "14-1-42-6",
                                                                    "14-1-F-1","14-1-F-2", "14-1-F-3","14-1-F-4","14-1-F-5","14-1-F-6",
                                                                    "14-1-37-1","14-1-37-2","14-1-37-3","14-1-37-4","14-1-37-5","14-1-37-6"))

phage_labels <- c(
  "14-1-37-1" = "37°C static 1",
  "14-1-37-2" = "37°C static 2",
  "14-1-37-3" = "37°C static 3",
  "14-1-37-4" = "37°C static 4",
  "14-1-37-5" = "37°C static 5",
  "14-1-37-6" = "37°C static 6",
  
  "14-1-F-1"  = "Fluc. static 1",
  "14-1-F-2"  = "Fluc. static 2",
  "14-1-F-3"  = "Fluc. static 3",
  "14-1-F-4"  = "Fluc. static 4",
  "14-1-F-5"  = "Fluc. static 5",
  "14-1-F-6"  = "Fluc. static 6",
  
  "14-1-42-1" = "42°C static 1",
  "14-1-42-2" = "42°C static 2",
  "14-1-42-3" = "42°C static 3",
  "14-1-42-4" = "42°C static 4",
  "14-1-42-5" = "42°C static 5",
  "14-1-42-6" = "42°C static 6"
)

phage14_variant_count =ggplot(breseq14_summary, aes(x = n, y = phage, fill = evol_group)) +
  geom_col() +
  xlim(0,25) +
  scale_y_discrete(
    limits = levels(breseq14_summary$phage),
    labels = phage_labels
  )+
  scale_fill_manual(values = cbPalette3) +
  labs(y = "Evolved population",
...
breseqL_summary = breseq_L_filtered %>%
  count(phage) %>%
  mutate(
    evol_group = case_when(
      grepl("37", phage) ~ "37°C static",
...
breseqL_summary$phage <- factor(breseqL_summary$phage, levels = c("LUZ19-42-1", "LUZ19-42-2", "LUZ19-42-3", "LUZ19-42-4", "LUZ19-42-5", "LUZ19-42-6",
                                                                  "LUZ19-F-1","LUZ19-F-2", "LUZ19-F-3","LUZ19-F-4","LUZ19-F-5","LUZ19-F-6",
                                                                  "LUZ19-37-1","LUZ19-37-2","LUZ19-37-3","LUZ19-37-4","LUZ19-37-5","LUZ19-37-6"))


luz19_variant_count = ggplot(breseqL_summary, aes(x = n, y = phage, fill = evol_group)) +
  geom_col() +
  xlim(0,25) +
  labs(y = "Evolved population",
       x = "Number of genetic variants",
...
phage14_variant_count + luz19_variant_count +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

ggsave("Fig_S2.png", width = 8, height = 4, dpi = 300)
...
ggsave("Fig_S3A.png", width = 4, height = 3, dpi = 300)
...
ggsave("Fig_S3B.png", width = 4, height = 3, dpi = 300)
...
ggsave("Figures/Fig_S4A.png", width = 4, height = 3, dpi = 300)
...
ggsave("Figures/Fig_S4B.png", width = 4, height = 3, dpi = 300)
