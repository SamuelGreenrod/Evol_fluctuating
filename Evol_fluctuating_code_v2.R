...
non_F_mutations <- breseq_14_filtered %>%
  filter(
    str_detect(phage, regex("anc", ignore_case = TRUE)) |
      evol_group %in% c("14-1-37", "14-1-42", "14-1-37-C", "14-1-42-C")
  ) %>%
  distinct(type, position, mutation)

# Step 2: remove rows that match any of those mutations
breseq_14_filtered <- breseq_14_filtered %>%
  anti_join(non_F_mutations, by = c("type", "position", "mutation"))
...
phage_data <- breseq_14_filtered %>%
  ...
  filter(!grepl("C", phage))  # Scale size for visualization

phage_data_37 <- phage_data %>%
  filter(evol_group == "14-1-37")

phage_data_37$phage <- factor(phage_data_37$phage, levels = c("14-1-37-1","14-1-37-2","14-1-37-3","14-1-37-4","14-1-37-5","14-1-37-6"))

phage_data_42 <- phage_data %>%
  filter(evol_group == "14-1-42")

phage_data_42$phage <- factor(phage_data_42$phage, levels = c("14-1-42-1","14-1-42-2","14-1-42-3","14-1-42-4","14-1-42-5","14-1-42-6"))


snp_37 <-  ggplot() +
  geom_segment(data = phage_data_37, 
               aes(x = 0, xend = 66298, y = phage, yend = phage), 
               color = "black", size = 0.5) +
  
  # Deletions > 1bp
  geom_rect(data = phage_data_37 %>% filter(type == "DEL" & mutation_length > 1), 
            aes(xmin = position, 
  ...
  # Insertion points
  geom_point(data = phage_data_37 %>% filter(type == "INS"),
             aes(x = position, y = phage, size = frequency, shape = "Insertion"), 
             fill = "red", color = "black", alpha = 0.6) +
  
  # SNP points
  geom_point(data = phage_data_37 %>% filter(type == "SNP"),
...
snp_42 <- ggplot() +
  geom_segment(data = phage_data_42, 
               aes(x = 0, xend = 66298, y = phage, yend = phage), 
               color = "black", size = 0.5) +
  
  # Deletions > 1bp
  geom_rect(data = phage_data_42 %>% filter(type == "DEL" & mutation_length > 1), 
            aes(xmin = position, 
                xmax = position + mutation_length, 
                ymin = as.numeric(phage) - frequency / 5, 
                ymax = as.numeric(phage) + frequency / 5, 
  ...
  # Insertion points
  geom_point(data = phage_data_42 %>% filter(type == "INS"),
             aes(x = position, y = phage, size = frequency, shape = "Insertion"), 
             fill = "red", color = "black", alpha = 0.6) +
  
  # SNP points
  geom_point(data = phage_data_42 %>% filter(type == "SNP"),
             aes(x = position, y = phage, size = frequency, shape = "SNP"), 
             fill = "darkorange", color = "black", alpha = 0.6) +
...
snp_37 / snp_42 / genes  + plot_layout(guides = "collect", heights = c(1, 1, 0.3))

ggsave("Figures/Fig4A1.png", width = 6, height = 4, dpi = 300)
...
phage14_pcoa = ggplot(data=Best_axes, aes(x=Axis.1,y=Axis.2, colour = evol_group))+ 
  ...
  theme(legend.title = element_text(colour="black", size=11,face="bold"),legend.text = element_text(size=11))+
...
ggsave("Figures/Fig_2a.png", width = 8, height = 3.5, dpi = 300)
